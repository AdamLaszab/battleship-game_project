...
void tlacPlochu(int hraciaPlocha[K][K],int hraciaPlochaProtivnik[K][K],int stav[5],int stav1[5]){ //funkcia vytlaci hracie plochy pre obidvoch hracov podla stavu
    ...
    for(int j=0;j<10;j++){
        ...
        printf("[%c]",'A'+j);
        for(int z=0;z<10;z++){
            if(hraciaPlocha[j][z]==0){ //ked je cislo v dvojrozmernom poli 0 tak je na mieste voda
                blue();
                printf("[~]");
            }
            if(hraciaPlocha[j][z]==1){ //ked je cislo v dvojrozmernom poli 1 tak je na mieste lod
                green();
                printf("[0]");
            }
            if(hraciaPlocha[j][z]%2!=0 && hraciaPlocha[j][z]>2 ){ //ked je cislo v dvojrozmernom poli neparne cislo vacsie ako 2 tak na tom mieste bola zasiahnuta lod
                red();
                printf("[*]");
            }
            if(hraciaPlocha[j][z]%2==0 && hraciaPlocha[j][z]>=2){ //ked je cislo v dvojrozmernom poli parne cislo vacsie ako 2 tak na mieste nebola lod ked utocili
                yellow();
                printf("[~]");
            }
        }
        if(j==0){
            ...
            if(stav[0]==0){ //stavy jednotlivych lodi menia farbu podla toho ci su potopene(0) alebo nie(1)
                red();
            }else{
                green();
            }
...
int zadajLod(int hraciaPlocha[K][K],int velkost,int* lode[17],int priebeh){  //funkcia nacita zaciatocnu poziciu lode a podla orientacie prida lod do dvojrozmerneho pola
    char riadok[100];
    char pismeno;
    int stlpec;
    char orientacia;
    ...
    pismeno=riadok[0];
    orientacia=riadok[2];
    if(strlen(riadok)==3){ //kontrola dlzky vstupu ak je cislo jednociferne
        stlpec = atoi(&riadok[1]);
    }else if(strlen(riadok)==4 && atoi(&riadok[1])==10 && atoi(&riadok[2])==0){ //kontrola dlzky vstupu ak je cislo dvojcifernce
        stlpec=10;
        orientacia=riadok[3];
    }else{
    ...
    if(pismeno>74){    //kontrola prveho pismena
        printf("Prve pismeno nesedi\n");
        return 0;
    }
    if(pismeno<65){ //kontrola prveho pismena
        printf("Prve pismeno nesedi\n");
        return 0;
    }
    if(stlpec>10){ //kontrola cisla
        printf("Cislo nesedi\n");
        return 0;
    }
    if(stlpec<0){ //kontrola cisla
        printf("Cislo nesedi\n");
        return 0;
    }
    if(orientacia!='r' && orientacia!='v'){ //kontrola orientacie
    ...
    stlpec=stlpec-1;
    if(orientacia=='r'){
        if((stlpec-1)+velkost>=K){ //kontrola ci lod bude siahat mimo hraciu plochu
            printf("Lod konci mimo hracej plochy\n");
            return 0;
        }
        for(int i=0;i<velkost;i++){
            if(hraciaPlocha[rad][stlpec+i]==1){ //kontrola ci sa lod bude prekryvat s inou lodou
                printf("Lod sa prekryva s inou lodou\n");
                return 0;
            }
        }
        for(int i=0;i<velkost;i++){ //zmeni 0 na 1ky podla velkosti a orientacie
            hraciaPlocha[rad][(stlpec)+i]=1;
            lode[priebeh+i]=&hraciaPlocha[rad][(stlpec)+i];
        }
    }
    if(orientacia=='v'){
        if((rad-1)+velkost>=K){ //kontrola ci lod bude siahat mimo hraciu plochu
            printf("Lod konci mimo hracej plochy\n");
            return 0;
        }
        for(int i=0;i<velkost;i++){
            if(hraciaPlocha[(rad)+i][(stlpec)]==1){ //kontrola ci sa lod bude prekryvat s inou lodou
                printf("Lod sa prekryva s inou lodou\n");
                return 0;
            }
        }
        for(int i=0;i<velkost;i++){
            ...
            lode[priebeh+i]=&hraciaPlocha[rad+i][(stlpec)]; //zmeni 0 na 1ky podla velkosti a orientacie
        }
    }
    priebeh=velkost;
    return priebeh;
...
void stavLodi(int* lode[17],int stavy[5]){ //podla poctu jednotiek zisti ci lode su potopene alebo stale v hre a podla toho upravi stavy
    int flag=0;
    for(int i=0;i<5;i++){ //ak lod este ma v sebe nejaku 1ku znamena ze neni potopena a podla toho sa meni jej stav
        if(*lode[i]==1){
            flag=1;
        }
    }
...
int main() {
    ...
    while(1) {
        if(x==0) {
            ...
            white();
            printf("Hrac 1 plocha:");
            for(int i=0;i<34;i++){
                printf(" ");
            }
            printf("Hrac 2 plocha:\n");
            ...
            printf("Koordinacie lodi zadavajte vo formate [riadok][stlpec][orientacia]\nkde stlpec je "
                   ...
                   "Zmeny sa objavia v prvom utocnom kole\n\n");
            while(priebeh==0) {
        ...
        if(x>0){
            ...
            if(flag==1){
                ...
                printf("KONECNE PLOCHY PRE OBOCH HRACOV:\n");
                white();
                printf("Hrac 1 plocha:");
                for(int i=0;i<34;i++){
                    printf(" ");
